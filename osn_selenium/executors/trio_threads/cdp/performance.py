...
class PerformanceCDPExecutor(
	...
	async def enable(self, time_domain: Optional[str] = None) -> None:
		return await self._sync_to_trio(self._enable_impl, time_domain=time_domain)
	
	async def get_metrics(self) -> List[Dict[str, Any]]:
		return await self._sync_to_trio(self._get_metrics_impl)
	
	async def set_time_domain(self, time_domain: str) -> None:
		return await self._sync_to_trio(self._set_time_domain_impl, time_domain=time_domain)
