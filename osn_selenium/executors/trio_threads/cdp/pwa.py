...
class PwaCDPExecutor(UnifiedPwaCDPExecutor, TrioThreadMixin, AbstractPwaCDPExecutor):
	...
	async def change_app_user_settings(
		...
		return await self._sync_to_trio(
				self._change_app_user_settings_impl,
	...
	async def get_os_app_state(self, manifest_id: str) -> Tuple[int, List[Dict[str, Any]]]:
		return await self._sync_to_trio(self._get_os_app_state_impl, manifest_id=manifest_id)
	...
	async def launch(self, manifest_id: str, url: Optional[str] = None) -> str:
		return await self._sync_to_trio(self._launch_impl, manifest_id=manifest_id, url=url)
	
	async def launch_files_in_app(self, manifest_id: str, files: List[str]) -> List[str]:
		return await self._sync_to_trio(self._launch_files_in_app_impl, manifest_id=manifest_id, files=files)
	
	async def open_current_page_in_app(self, manifest_id: str) -> None:
		return await self._sync_to_trio(self._open_current_page_in_app_impl, manifest_id=manifest_id)
	
	async def uninstall(self, manifest_id: str) -> None:
		return await self._sync_to_trio(self._uninstall_impl, manifest_id=manifest_id)
