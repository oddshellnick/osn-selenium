...
class ProfilerCDPExecutor(
	...
	async def get_best_effort_coverage(self) -> List[Dict[str, Any]]:
		return await self._sync_to_trio(self._get_best_effort_coverage_impl)
	
	async def set_sampling_interval(self, interval: int) -> None:
		return await self._sync_to_trio(self._set_sampling_interval_impl, interval=interval)
	
	async def start(self) -> None:
		return await self._sync_to_trio(self._start_impl)
	...
	async def start_precise_coverage(
		...
		return await self._sync_to_trio(
				self._start_precise_coverage_impl,
	...
	async def stop(self) -> Dict[str, Any]:
		return await self._sync_to_trio(self._stop_impl)
	
	async def stop_precise_coverage(self) -> None:
		return await self._sync_to_trio(self._stop_precise_coverage_impl)
	
	async def take_precise_coverage(self) -> Tuple[List[Dict[str, Any]], float]:
		return await self._sync_to_trio(self._take_precise_coverage_impl)
