...
class MemoryCDPExecutor(UnifiedMemoryCDPExecutor, TrioThreadMixin, AbstractMemoryCDPExecutor):
	...
	async def forcibly_purge_java_script_memory(self) -> None:
		return await self._sync_to_trio(self._forcibly_purge_java_script_memory_impl)
	
	async def get_all_time_sampling_profile(self) -> Dict[str, Any]:
		return await self._sync_to_trio(self._get_all_time_sampling_profile_impl)
	
	async def get_browser_sampling_profile(self) -> Dict[str, Any]:
		return await self._sync_to_trio(self._get_browser_sampling_profile_impl)
	
	async def get_dom_counters(self) -> Tuple[int, int, int]:
		return await self._sync_to_trio(self._get_dom_counters_impl)
	
	async def get_dom_counters_for_leak_detection(self) -> List[Dict[str, Any]]:
		return await self._sync_to_trio(self._get_dom_counters_for_leak_detection_impl)
	...
	async def prepare_for_leak_detection(self) -> None:
		return await self._sync_to_trio(self._prepare_for_leak_detection_impl)
	
	async def set_pressure_notifications_suppressed(self, suppressed: bool) -> None:
		return await self._sync_to_trio(
				self._set_pressure_notifications_suppressed_impl,
				suppressed=suppressed
		)
	
	async def simulate_pressure_notification(self, level: str) -> None:
		return await self._sync_to_trio(self._simulate_pressure_notification_impl, level=level)
