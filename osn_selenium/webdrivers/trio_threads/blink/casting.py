...
class BlinkCastingMixin(BlinkBaseMixin, AbstractBlinkCastingMixin):
	...
	async def get_sinks(self) -> List[Dict[str, Any]]:
		return await self._sync_to_trio(self.driver.get_sinks)
	
	@requires_driver
	async def set_sink_to_use(self, sink_name: str) -> Dict[str, Any]:
		return await self._sync_to_trio(self.driver.set_sink_to_use, sink_name=sink_name)
	
	@requires_driver
	async def start_desktop_mirroring(self, sink_name: str) -> Dict[str, Any]:
		return await self._sync_to_trio(self.driver.start_desktop_mirroring, sink_name=sink_name)
	
	@requires_driver
	async def start_tab_mirroring(self, sink_name: str) -> Dict[str, Any]:
		return await self._sync_to_trio(self.driver.start_tab_mirroring, sink_name=sink_name)
	
	@requires_driver
	async def stop_casting(self, sink_name: str) -> Dict[str, Any]:
		return await self._sync_to_trio(self.driver.stop_casting, sink_name=sink_name)
