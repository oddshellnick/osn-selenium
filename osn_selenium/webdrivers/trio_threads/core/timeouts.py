...
class CoreTimeoutsMixin(CoreBaseMixin, AbstractCoreTimeoutsMixin):
	...
	async def implicitly_wait(self, time_to_wait: float) -> None:
		await self.sync_to_trio(sync_function=self.driver.implicitly_wait)(time_to_wait=time_to_wait)
	
	@requires_driver
	async def set_page_load_timeout(self, time_to_wait: float) -> None:
		await self.sync_to_trio(sync_function=self.driver.set_page_load_timeout)(time_to_wait=time_to_wait)
	
	@requires_driver
	async def set_script_timeout(self, time_to_wait: float) -> None:
		await self.sync_to_trio(sync_function=self.driver.set_script_timeout)(time_to_wait=time_to_wait)
	...
	async def timeouts(self) -> Any:
		return await self.sync_to_trio(sync_function=lambda: self.driver.timeouts)()
	...
	async def set_driver_timeouts(
		...
		await self.sync_to_trio(sync_function=_set)()
