...
def yield_package_item_way(name: Union[str, Iterable[str]]) -> Generator[str, Any, None]:
    ...
    if (
            ...
            and (
                    not isinstance(name, (list, set, tuple))
                    or not all(isinstance(item, str) for item in name)
            )
    ):
        raise TypeError(f"Wrong name type ({type(name).__name__})! Must be str or Iterable[str]!")
...
def validate_target_event_filter(filter_: Optional[Sequence[Dict[str, Any]]]) -> "TargetsFilters":
    ...
    if len(set(all_excluded_types) & set(all_included_types)) > 0:
        raise ValueError(f"Duplicate types in target filters! ({set(all_excluded_types) & set(all_included_types)})")

    other_types = any(
            type_filter.get("type", None) is None and not type_filter.get("exclude", True)
            for type_filter in filter_
    )
...
def validate_target_event(event: Any, filter_: "TargetsFilters") -> Optional[bool]:
    ...
    if hasattr(event, "target_info") and hasattr(event.target_info, "type_"):
        result = validate_target_type(
            type_=event.target_info.type_,
            filter_=filter_
        )
...
async def execute_cdp_command(
    ...
    Args:
        self ("BaseTargetMixin"): The `BaseTargetMixin` instance through which the command is executed.
