...
async def _handle_auth_required(
    ...
    try:
        ...
        for action_func_name in chosen_actions_func_names:
            ...
            kwargs = await chosen_func.kwargs_func(
                self,
                chosen_func.parameters_handlers,
                event
            )
...
async def _handle_request_paused(
    ...
    try:
        ...
        for action_func_name in chosen_actions_func_names:
            ...
            kwargs = await chosen_action_func.kwargs_func(
                self,
                chosen_action_func.parameters_handlers,
                event
            )
            await self.log(level="INFO", message=f"Kwargs for '{action_func_name}': '{kwargs}'")
...
request_paused_choose_action_func_type = Callable[
    [DevToolsTarget, Any],
    Sequence[request_paused_actions_literal]
]
auth_required_choose_action_func_type = Callable[
    [DevToolsTarget, Any],
    Sequence[auth_required_actions_literal]
]
handle_request_paused_func_type = Callable[
    [DevToolsTarget, RequestPausedSettings, Any],
    Awaitable[None]
]
handle_auth_required_func_type = Callable[
    [DevToolsTarget, AuthRequiredSettings, Any],
    Awaitable[None]
]
